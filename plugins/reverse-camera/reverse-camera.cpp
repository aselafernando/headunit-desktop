...
Q_LOGGING_CATEGORY(LOG_PLUGIN_REVERSECAMERA, "plugins.reverse-camera")
...
void ReversingCamera::videoItemLoaded(QQuickItem *vi) {
    ...
    if(videoItem) {
        ...
        if(rootObject) {
            qCDebug(LOG_PLUGIN_REVERSECAMERA) << "Root Object Found";
...
void ReversingCamera::settingsChanged(const QString &key, const QVariant &) {
    if (key == "gs_src") {
         qCDebug(LOG_PLUGIN_REVERSECAMERA) << "gs_src";
    } else if(key == "gs_pipeline") {
         qCDebug(LOG_PLUGIN_REVERSECAMERA) << "gs_pipeline";
    }
}
...
void ReversingCamera::destroyPipeline() {
   qCDebug(LOG_PLUGIN_REVERSECAMERA) << "destroyPipeline()";
...
void ReversingCamera::setupPipeline() {
    ...
    qCDebug(LOG_PLUGIN_REVERSECAMERA) << "GStreamer Src:" << gs_src;
    qCDebug(LOG_PLUGIN_REVERSECAMERA) << "GStreamer Pipeline:" << gs_pipeline;
    ...
    if(gs_src != "") {
        ...
        if (error) {
            qCDebug(LOG_PLUGIN_REVERSECAMERA) << error->message;
            g_error_free(error);
            destroyPipeline();
            return;
        }
    ...
    if(!processPipeline) {
        qCDebug(LOG_PLUGIN_REVERSECAMERA) << "gstreamer process pipeline error in configuration!";
        destroyPipeline();
        return;
    }
    ...
    if(!glupload) {
        qCDebug(LOG_PLUGIN_REVERSECAMERA) << "gstreamer glupload missing";
        destroyPipeline();
        return;
    }

    if(!glcolorconvert) {
        qCDebug(LOG_PLUGIN_REVERSECAMERA) << "gstreamer glcolorconvert missing";
        destroyPipeline();
        return;
    }
    */
    if(!sink) {
        qCDebug(LOG_PLUGIN_REVERSECAMERA) << "gstreamer qml6glsink missing?";
        destroyPipeline();
        return;
    }

    if(src) {
        qCDebug(LOG_PLUGIN_REVERSECAMERA) << "Linking with delayed pads";
        ...
        g_signal_connect(src, "pad-added", G_CALLBACK(+[](__attribute__((unused)) GstElement *src, GstPad *pad, GstElement *sink) {
                qCDebug(LOG_PLUGIN_REVERSECAMERA) << "GStreamer src pad-added";
        ...
        qCDebug(LOG_PLUGIN_REVERSECAMERA) << "Linking with present pads";
        gst_bin_add_many(GST_BIN(pipeline), processPipeline, sink, NULL);
    }

    gst_element_link_many(processPipeline, sink, NULL);

    qCDebug(LOG_PLUGIN_REVERSECAMERA) << "Pipeline Made";
}

void ReversingCamera::onSettingsPageDestroyed(){
    //setupPipeline();
    qCDebug(LOG_PLUGIN_REVERSECAMERA) << "Settings Destroyed";
    destroyPipeline();
    setupPipeline();
}
